...
const Wave = ({ side, children, position, isTransitioning }: WaveProps) => {
  const R = useDerivedValue(() => {
    const value = Math.min(position.x.value - MIN_LEDGE, WIDTH - position.x.value);
    return value > 0 ? value : MIN_LEDGE;

  });
  const ledge  = useDerivedValue(()=>{
     const baseLadge = Math.max(0, position.x.value - MIN_LEDGE);
     return withSpring(isTransitioning.value ? position.x.value : baseLadge, {
      stiffness: 250,
     });
  })
